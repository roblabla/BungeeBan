...
public class TempBanIpCommand extends Command {
	...
	public TempBanIpCommand(IBanStore banstore) {
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if (args.length < 3) {
			...
			sender.sendMessage(ChatColor.RED + "/tempbanip <ip> <bantime> [reason]");
			return;
		} else {
			try {
		...
		if (args.length == 2) {
			banstore.tempbanIP(args[0], player.getServer().getInfo().getName(), sender.getName(), "", date);
		} else {
			StringBuilder reasonBuilder = new StringBuilder();
			for (int i = 2;i < args.length; i++) {
				reasonBuilder.append(args[i]);
			}
			banstore.tempbanIP(args[0],player.getServer().getInfo().getName(), sender.getName(),
