...
public class GTempBanCommand extends Command {
	...
	public GTempBanCommand(IBanStore banstore) {
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if (args.length < 2) {
			...
			sender.sendMessage(ChatColor.RED + "/gtempban <username> <bantime> [reason]");
			return;
		} else {
			try {
		...
		if (args.length == 2) {
			banstore.gtempban(args[0], sender.getName(), "", date);
		} else {
			StringBuilder reasonBuilder = new StringBuilder();
			for (int i = 2;i < args.length; i++) {
				reasonBuilder.append(args[i]);
			}
			banstore.gtempban(args[0],sender.getName(),reasonBuilder.toString(), date);
		}
	}
}
